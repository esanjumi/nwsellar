...
test('Automate 1000 checkouts with random names and emails', async ({ page }) => {
  ...
  for (let i = 0; i < TOTAL_CHECKOUTS; i++) {
    ...
    try {
      ...
      await page.getByRole('button', { name: 'Get now' }).click();

      // Wait for item details page to load (Adjust selector if necessary)
      ...
      // Optionally, continue to the next checkout if an error occurs
      continue;
    }
  }
